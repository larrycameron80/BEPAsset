...
//    })
  ...
  */
  ...
  listProposalStatus(req, res, next) {

    if(!req.params.address) {
      res.status(404)
      res.body = { 'status': 404, 'success': false, 'result': 'address is required' }
      return next(null, req, res, next)
    }
    ...
    bnb.getListProposals(req.params.address, req.params.symbol, (err, proposalId) => {
      if (err) {
        res.status(400)
        res.body = { 'status': 400, 'success': false, 'result': err }
        return next(null, req, res, next)
      }

      if (!proposalId) {
        res.status(205)
        res.body = { 'status': 200, 'success': true, 'result': proposalId }
        return next(null, req, res, next)
      }
